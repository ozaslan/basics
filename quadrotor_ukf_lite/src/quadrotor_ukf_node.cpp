...
double particle_x, particle_var;
...
void imu_callback(const sensor_msgs::Imu::ConstPtr& msg)
{
	...
	else if (quadrotorUKF.ProcessUpdate(u, msg->header.stamp))  // Process Update
	{
		...
		// Save particle position and variance as well
		odomUKF.pose.covariance[0+4*6] = particle_x;
		odomUKF.pose.covariance[0+5*6] = particle_var;
	
		pubUKF.publish(odomUKF); 
	}
	//ROS_INFO("Exit imu_callback");
}

void slam_callback(const nav_msgs::Odometry::ConstPtr& msg)
{
	//ROS_INFO("Quadrotor_UKF - In slam_callback");
	// Get orientation
	colvec q(4);
	...
	particle_x   = msg->pose.covariance[0+4*6];
	particle_var = msg->pose.covariance[0+5*6];
...
int main(int argc, char** argv)
{
	...
	ros::Subscriber subImu  = n.subscribe("imu" ,      100, imu_callback);
	ros::Subscriber subSLAM = n.subscribe("odom_slam", 100, slam_callback);
	ros::Subscriber subGPS  = n.subscribe("odom_gps",  100, gps_callback);
	pubUKF = n.advertise<nav_msgs::Odometry>("/control_odom", 100);
	ros::spin();

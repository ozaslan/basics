/*
   ...
   In addition to 'laser_proc_node' this node demo's the performance
   of the CSM algorithm and noise filtering.
 */
...
int main(int argc, char* argv[]){
	...
	ros::init(argc, argv, "laser_proc_align");
	ros::NodeHandle n("~");
...
void lidar_callback(const sensor_msgs::LaserScan &msg){
	...
	static Eigen::Matrix4d icp_trans = Eigen::Matrix4d::Identity();

	in_lidar_msg = msg;

	// Get the laser data for scan alignment before overwriting.
	vector<int> mask0 = laser_procc.get_mask();
	vector<double> ranges0 = laser_procc.get_ranges();
	vector<double> linearity0 = laser_procc.get_linearity_rates();
	...
	// Get the data of the second lidar reading for scan alignment
	vector<int> mask1 = laser_procc.get_mask();
	vector<double> ranges1 = laser_procc.get_ranges();
	vector<double> linearity1 = laser_procc.get_linearity_rates();

	for(int i = 0 ; i < (int)mask0.size() ; i++)
		if(linearity0[i] > 0.4)
			mask0[i] = 0;

	vector<double> ths(mask1.size());
	for(int i = 0 ; i < (int)mask1.size() ; i++){
		ths[i] = in_lidar_msg.angle_min + in_lidar_msg.angle_increment * i;
		if(linearity1[i] > 0.4)
			mask1[i] = 0;
	}

	// Call the utils::laser::register_scan(...) function.
	//cout << "mask0.size() = " << mask0.size() << endl;
	//cout << "mask1.size() = " << mask1.size() << endl;
	if(mask0.size() != 0){
		Eigen::Vector5d icp_res = utils::laser::register_scan(ranges0, mask0, ths, ranges1, mask1, ths);
		Eigen::Matrix4d dicp_trans = Eigen::Matrix4d::Identity();
